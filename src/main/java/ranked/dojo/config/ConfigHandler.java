package ranked.dojo.config;

import lombok.Getter;
import ranked.dojo.Dojo;
import ranked.dojo.config.values.EnumDefaultConfigMessages;
import ranked.dojo.config.values.EnumDefaultConfigSettings;
...
public class ConfigHandler {
    ...
    public void loadConfigs(String[] configNames) {
        for (String configName : configNames) {
            File configFile = new File(Dojo.getInstance().getDataFolder(), configName + ".yml");
    ...
    private void createDefaultConfig(File configFile, String configName) {
        try {
            ...
            BukkitUtils.log("Could not create the " + configFile.getName() + " file: " + e.getMessage());
        }
    }
    ...
    public void saveConfig(File file, FileConfiguration config) {
        try {
            ...
            BukkitUtils.log("Could not save the " + file.getName() + " file: " + e.getMessage());
        }
    }
}
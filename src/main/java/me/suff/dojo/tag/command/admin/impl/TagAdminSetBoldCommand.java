...
public class TagAdminSetBoldCommand extends BaseCommand {
    @Command(name = "tagadmin.setbold", permission = "dojo.ta.setbold", inGameOnly = false)
    @Override
    public void onCommand(CommandArgs command) {
        ...
        if (args.length < 2) {
            sender.sendMessage(CC.translate("&cUsage: &e/tagadmin setbold (name) (true/false)"));
            return;
        }
        ...
        sender.sendMessage(CC.translate("&aSuccessfully set the bold of the tag &e" + name + "&r&a to &f" + bold + "&r&a."));
    }
}

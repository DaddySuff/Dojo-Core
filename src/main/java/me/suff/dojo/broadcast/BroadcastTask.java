package me.suff.dojo.broadcast;
    ...
    public void run() {
        ...
        for (String line : this.broadcasts.get(ThreadLocalRandom.current().nextInt(this.broadcasts.size())).getLines()) {
            for (Player player : Bukkit.getOnlinePlayers()) {
                Rank playerRank = Dojo.getInstance().getProfileRepository().getProfileWithNoAdding(player.getUniqueId()).getHighestRankBasedOnGrant();
